...
export default function QuickSearch({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-start justify-center pt-[15vh] z-50" onClick={onClose}>
      <div className="glass rounded-xl w-full max-w-lg shadow-2xl shadow-black/50 border-white/10" onClick={e => e.stopPropagation()}>
        <div className="flex items-center border-b border-white/[0.06] px-4">
          ...
          <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)}
            ...
            className="flex-1 bg-transparent px-3 py-3 text-sm text-zinc-200 outline-none placeholder-zinc-600" />
          <button onClick={onClose} className="text-zinc-600 hover:text-zinc-300 transition-colors"><X size={16} /></button>
        </div>
        <div className="max-h-80 overflow-y-auto p-2">
          {results.people.length > 0 && (
            <div className="mb-2">
              <span className="text-xs text-zinc-600 px-2">People</span>
              {results.people.map(p => (
                <button key={p.id} onClick={() => go(`/people/${p.id}`)}
                  className="w-full text-left px-3 py-2 rounded text-sm text-zinc-300 hover:bg-white/[0.05] transition-colors">{p.display_name} <span className="text-zinc-600">{p.role}</span></button>
              ))}
            </div>
          )}
          {results.projects.length > 0 && (
            <div className="mb-2">
              <span className="text-xs text-zinc-600 px-2">Projects</span>
              {results.projects.map(p => (
                <button key={p.id} onClick={() => go(`/projects/${p.id}`)}
                  className="w-full text-left px-3 py-2 rounded text-sm text-zinc-300 hover:bg-white/[0.05] transition-colors">{p.name} {p.short_code && <span className="text-zinc-600">[{p.short_code}]</span>}</button>
              ))}
            </div>
          )}
          {results.items.length > 0 && (
            <div>
              <span className="text-xs text-zinc-600 px-2">Items</span>
              {results.items.map(i => (
                <div key={i.id} className="px-3 py-2 rounded text-sm text-zinc-500 hover:bg-white/[0.05] transition-colors">{i.raw_text.slice(0, 80)}</div>
              ))}
            </div>
          )}
          {query && results.people.length === 0 && results.projects.length === 0 && results.items.length === 0 && (
            <div className="text-center text-zinc-700 py-4 text-sm">No results</div>
          )}
        </div>
      </div>
    </div>
